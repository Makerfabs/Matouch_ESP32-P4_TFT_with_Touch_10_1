 ...
 */
 ...
 bool Camera::run(void)
 {
     ...
     for(int i=0;i<2;i++)
     {
         ppa_buffer[i] = (uint8_t *)heap_caps_aligned_alloc(data_cache_line_size, 798 *1239 *2, MALLOC_CAP_SPIRAM);
         if(!ppa_buffer[i])
         {
...
//  }
                                        ...
                                        size_t camera_buf_len)
 {
     ...
     //         .buffer_size = ALIGN_UP_BY(1280 * 800 * 2, data_cache_line_size),
     //         .pic_w = 800,
     //         .pic_h = 1280,
     //         .block_offset_x = 0,
     //         .block_offset_y = 0,
     ...
     ppa_srm_oper_config_t cam_srm_config = {
         ...
         .out = {
             .buffer = ppa_buffer[camera_buf_index],
             .buffer_size = ALIGN_UP_BY(798 * 1239 * (APP_VIDEO_FMT == APP_VIDEO_FMT_RGB565 ? 2 : 3), data_cache_line_size),
             // .buffer_size = 1280 *800 *2,
             .pic_w = 798,
             .pic_h = 1239,
             .block_offset_x = 0,
             .block_offset_y = 0,
     ...
     // int res = ppa_do_scale_rotate_mirror(ppa_srm_handle, &oper_config_out);
     if (res != ESP_OK) {
         ESP_LOGE(TAG, "ppa_do_scale_rotate_mirror 2 failed with error 0x%x", res);
     }
 
     if (!(xEventGroupGetBits(camera_event_group) & CAMERA_EVENT_DELETE)) {
         bsp_display_lock(0);
         lv_canvas_set_buffer(ui_ImageCameraShotImage, ppa_buffer[camera_buf_index], 798, 1239, LV_IMG_CF_TRUE_COLOR);
         lv_refr_now(NULL);
         bsp_display_unlock();
     }
