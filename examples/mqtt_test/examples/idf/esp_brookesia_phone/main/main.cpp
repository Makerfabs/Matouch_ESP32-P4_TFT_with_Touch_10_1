...
extern "C" void app_main(void)
{
    ...
    // 添加导航栏
    phone_stylesheet->home.navigation_bar.visual_mode = ESP_BROOKESIA_NAVIGATION_BAR_VISUAL_MODE_SHOW_FIXED;
    ...
    // network test
    NetworkApp *network_app = new NetworkApp();
    assert(network_app != nullptr && "Failed to create network_app");
    assert((phone->installApp(network_app) >= 0) && "Failed to begin network_app");

    // DHT11 sensor app
    ESP_LOGI(TAG, "Creating SensorApp...");
    SensorApp *sensor_app = new SensorApp();
    assert(sensor_app != nullptr && "Failed to create sensor_app");
    ESP_LOGI(TAG, "Installing SensorApp...");
    int ret = phone->installApp(sensor_app);
    if (ret >= 0) {
        ESP_LOGI(TAG, "SensorApp installed successfully (index: %d)", ret);
    } else {
        ESP_LOGE(TAG, "Failed to install SensorApp (error: %d)", ret);
    }
